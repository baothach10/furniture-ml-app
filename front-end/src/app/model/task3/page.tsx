"use client"
import {handleTask3ImageSubmit} from '@/app/actions/submit-images';
...
const FileUploadForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true)
        if (selectedFile) {
            const response = await handleTask3ImageSubmit(selectedFile);
            setLoading(false)
        ...
        <div>
            ...
            <div className='flex flex-row'>
                {recommendations && recommendations.map((rec, index) => (
                    <Image width={200} height={200} src={'http://localhost:8001/' + rec} alt='recomendations'
                           key={index}/>
                ))}
            </div>
        </div>
