...
class ImageRecommender:
    def __init__(self, cnn_model_path, feature_data_path):
    ...
    def recommend_images(self, img_path, top_n=10, show_image=False):
        ...
        if show_image:
            ...
            for file in indices[0]:
                img = cv2.imread(self.file_paths[file])
                RGB_img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                images.append(RGB_img)

                print(self.categories[file])
        ...
        return self.file_paths[indices[0]]
...
def save_recommender(cnn_model_path, feature_data_path, save_path):
    recommender = ImageRecommender(cnn_model_path, feature_data_path)
...
# save_path = r'./src/app/model/task2_recommender.pkl'
# save_recommender(cnn_model_path, feature_data_path, save_path)